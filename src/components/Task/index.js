...
class Task extends React.Component {
  ...
  componentWillReceiveProps(nextProps) {
    const {task, timer, setTimer} = nextProps;
  ...
  handleSetTimer = () => {
    ...
    if (timer.taskId !== task.id) 
      setTimer({taskId: task.id, taskText: task.text, 
        startTime: new Date().toISOString(), duration: 0});
  };

  stopTimer = () => {
    const {task, timer, setTimer, addTimerData} = this.props;
    addTimerData({taskText: timer.taskText, duration: timer.duration});
    if (timer.taskId === task.id) 
